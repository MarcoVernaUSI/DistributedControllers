...
class Agent:
    def __init__(self, initial_pose, number, width=0.06, min_range=0.0, max_range=9999999.0):
    ...
    def observe(self, agents_list, l, idx):
        ...
        return np.array(
            [np.clip(left, self.min_range, self.max_range), np.clip(right, self.min_range, self.max_range)]), np.array(
            [vel_left, vel_right])
