...
class ComNet(nn.Module):
    ...
    def single_step(self, xs, comm, only_comm = False):
    	if only_comm:
        	input = torch.from_numpy(comm).float()
        else:
...
class ComNetL(ComNet):
    def __init__(self, N, sync = Sync.sequential, module = SNetL,
                 input_fn=input_from):
...
class ComNetLnoSensing(ComNet):
    def __init__(self, N, sync = Sync.sequential, module = SNetLnoSensing,
                 input_fn=input_from_no_sensing):
...
class ComNetLnoSensingN(ComNetLnoSensing):
    ...
    def controller(self, sync= Sync.sync):
        N = self.N
        if sync == None:
            sync = self.sync
        tmp = list(range(self.N))
