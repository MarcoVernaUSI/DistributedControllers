...
#			t=t+1
	...
	def step(self, sensing, comm,net):
		...
		if self.cmd:  ##### Questo  da rivedere
			...
			#############Syncronizing
			self.state = None 
			self.received_comm = None
			sensing = self.state
			comm = self.received_comm
			while (sensing is None) or (comm is None):
				sensing = self.state
				comm = self.received_comm
			#	if sensing is None:
			#		print(self.thymio_name, ': waiting for sensors')
			#	if sensing is None:
			#		print(self.thymio_name, ': waiting for communication')
